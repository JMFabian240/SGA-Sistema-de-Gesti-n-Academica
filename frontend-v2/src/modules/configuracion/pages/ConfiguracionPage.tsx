...
import { 
  ...
  AlertTriangle, Check, Info, FileText, CheckCircle 
} from 'lucide-react';
...
export function ConfiguracionPage() {
  ...
  const handleEjecutarCierre = () => {
    if (window.confirm('¡ATENCIÓN! La operación de Cierre de Ciclo es irreversible y congelará el historial académico actual. ¿Desea proceder?')) {
      ...
      <div className="mt-6">
        ...
        {activeTab === 'cierre' && (
          <div className="max-w-2xl mx-auto space-y-6">
            {/* Warning Callout */}
            <div className="p-4 bg-red-50 text-red-800 rounded-xl border border-red-100 flex items-start gap-3 animate-in slide-in-from-top duration-300">
              <AlertTriangle className="shrink-0 mt-0.5 text-red-600" size={20} />
              <div>
                <h4 className="font-bold text-sm">Operación Irreversible — Proceda con extrema precaución</h4>
                <p className="text-xs text-red-700 mt-1">El cierre de ciclo bloquea el año escolar activo y genera los reportes consolidados históricos.</p>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-6">
              <div className="border-b border-gray-100 pb-4">
                <h3 className="text-lg font-bold text-navy-800">Cierre de Ciclo Escolar</h3>
                <p className="text-xs text-gray-500">Esta acción archiva expedientes e inicializa el periodo de transición.</p>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3 p-3 bg-slate-50 border border-gray-100 rounded-xl">
                  ...
                  <span className="text-xs text-gray-600">Se archivarán todos los expedientes del ciclo actual.</span>
                </div>
                <div className="flex items-center gap-3 p-3 bg-slate-50 border border-gray-100 rounded-xl">
                  <Check className="text-green-600 shrink-0" size={16} />
                  <span className="text-xs text-gray-600">Se bloqueará la edición de calificaciones correspondientes a periodos pasados.</span>
                </div>
                <div className="flex items-center gap-3 p-3 bg-slate-50 border border-gray-100 rounded-xl">
                  <Check className="text-green-600 shrink-0" size={16} />
                  <span className="text-xs text-gray-600">Los alumnos con adeudos pendientes serán retenidos automáticamente (no promovibles).</span>
                </div>
              </div>

              {/* Destination Cycle Info */}
              <div className="flex justify-between items-center p-4 bg-navy-50 rounded-xl border border-navy-100">
                <div className="flex items-center gap-2">
                  <FileText className="text-navy-700" size={16} />
                  <span className="text-xs text-navy-800 font-bold uppercase">Ciclo de Destino:</span>
                </div>
                <span className="text-sm font-bold text-navy-900">Ciclo Escolar Siguiente</span>
              </div>

              {/* Success Notification */}
              {cierreCompletado && (
                <div className="p-4 bg-green-50 text-green-800 border border-green-100 rounded-xl text-center font-bold text-sm flex items-center justify-center gap-2 animate-in fade-in">
                  <CheckCircle size={18} /> ¡El cierre del ciclo escolar fue ejecutado exitosamente!
                </div>
              )}
